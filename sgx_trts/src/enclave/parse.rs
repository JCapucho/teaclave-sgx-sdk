...
unsafe fn relocate_elf_rela(elf: &ElfFile, sym_offset: u64, rel_offset: u64, rel_size: u64) {
    let sym_table =
        (AsSlice::as_slice(elf.input).as_ptr() as usize + sym_offset as usize) as *const u8;
    ...
    AsSlice::as_slice(rel_array.get_array())
        .iter()
        .for_each(|rel| {
            let reloc_addr = (AsSlice::as_slice(elf.input).as_ptr() as usize
                + rel.get_offset() as usize) as *mut u64;

            match rel.get_type() {
                sections::R_X86_64_RELATIVE => {
                    *reloc_addr = AsSlice::as_slice(elf.input).as_ptr() as u64 + rel.get_addend();
                }
                sections::R_X86_64_GLOB_DAT
                | sections::R_X86_64_JMP_SLOT
                | sections::R_X86_64_64 => {
                    if let Some(sym) = get_sym(sym_table, rel.get_symbol_table_index()) {
                        *reloc_addr = AsSlice::as_slice(elf.input).as_ptr() as u64
                            + sym.value()
                            + rel.get_addend();
                    }
                }
